...
const mockLeads = [
  { id: '1', name: 'Priya Menon', phone: '+91 98765 43210', score: 9.1, bucket: 'HOT', status: 'COMPLETED', company: 'Infosys' },
  { id: '2', name: 'Arjun Sharma', phone: '+91 87654 32109', score: 8.5, bucket: 'HOT', status: 'COMPLETED', company: 'Wipro' },
  { id: '3', name: 'Neha Kapoor', phone: '+91 76543 21098', score: 6.2, bucket: 'WARM', status: 'COMPLETED', company: 'TCS' },
  { id: '4', name: 'Rohit Verma', phone: '+91 65432 10987', score: 4.8, bucket: 'WARM', status: 'CALLING', company: 'HCL Tech' },
  { id: '5', name: 'Anjali Singh', phone: '+91 54321 09876', score: 2.1, bucket: 'COLD', status: 'COMPLETED', company: 'Cognizant' },
  { id: '6', name: 'Vikram Nair', phone: '+91 43210 98765', score: null, bucket: null, status: 'PENDING', company: 'Tech Mahindra' },
  { id: '7', name: 'Kavya Reddy', phone: '+91 32109 87654', score: 7.3, bucket: 'HOT', status: 'COMPLETED', company: 'Mphasis' },
];
...
const cards = [
  { key: "totalLeads", label: "Total Leads", icon: Users, color: "#0F3D3E", bg: "linear-gradient(135deg,rgba(15,61,62,0.07),rgba(15,61,62,0.02))", border: "rgba(15,61,62,0.15)", iconBg: "rgba(15,61,62,0.08)", trend: "+12%", sparkData: [18, 22, 19, 28, 25, 32, 30, 38, 35, 42] },
  { key: "callsInitiated", label: "Calls Made", icon: PhoneCall, color: "#D4AF37", bg: "linear-gradient(135deg,rgba(212,175,55,0.07),rgba(212,175,55,0.02))", border: "rgba(212,175,55,0.15)", iconBg: "rgba(212,175,55,0.08)", trend: "+8%", sparkData: [14, 16, 20, 18, 24, 22, 28, 26, 30, 34] },
  { key: "hotLeads", label: "Hot Leads", icon: Flame, color: "#1F8A70", bg: "linear-gradient(135deg,rgba(31,138,112,0.07),rgba(31,138,112,0.02))", border: "rgba(31,138,112,0.15)", iconBg: "rgba(31,138,112,0.08)", trend: "+24%", sparkData: [8, 12, 10, 16, 14, 20, 22, 18, 26, 30] },
  { key: "conversionRate", label: "Conversion", icon: Target, color: "#A67C2E", bg: "linear-gradient(135deg,rgba(166,124,46,0.07),rgba(166,124,46,0.02))", border: "rgba(166,124,46,0.15)", iconBg: "rgba(166,124,46,0.08)", trend: "+3.2 pts", sparkData: [28, 30, 29, 32, 31, 34, 33, 35, 34, 36] },
];
...
function useTilt3D() {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const glow = innerRef.current.querySelector('.card-glow') as HTMLElement | null;
    if (glow) {
  ...
  const handleMouseLeave = useCallback(() => {
    if (innerRef.current) {
      innerRef.current.style.transform = 'perspective(900px) rotateX(0) rotateY(0) scale3d(1,1,1)';
    }
  }, []);
  ...
  return (
    <motion.div {...fadeUp(delay)}>
      <div ref={ref} className="tilt-card" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
        <div
          ref={innerRef}
          className="stat-card relative p-5 cursor-default rounded-[22px] overflow-hidden"
          style={{
            background: card.bg,
            border: `1px solid ${card.border}`,
            boxShadow: `0 4px 20px ${card.color}08, 0 1px 4px rgba(0,0,0,0.04)`,
        ...
        >
          ...
          <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full pointer-events-none"
            style={{ background: card.color, opacity: 0.07, filter: "blur(24px)" }} />

          {/* Gold shimmer bottom bar */}
          <div className="absolute bottom-0 left-4 right-4 h-[1px]"
            style={{ background: `linear-gradient(90deg,transparent,${card.color}25,transparent)` }} />

          {/* Header row */}
          <div className="flex items-start justify-between mb-3 relative z-10">
            <div className="text-[12px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>{card.label}</div>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
              style={{ backgroundColor: card.iconBg, border: `1px solid ${card.border}` }}>
    ...
    <div className="relative flex items-center justify-center">
      <svg width="96" height="96" className="-rotate-90">
        ...
        <motion.circle cx={cx} cy={cy} r={r} fill="none" stroke="#1F8A70" strokeWidth="12"
          ...
          transition={{ duration: 1.4, delay: 0.3, ease: [0.16, 1, 0.3, 1] }} strokeLinecap="round" />
        <motion.circle cx={cx} cy={cy} r={r} fill="none" stroke="#D4AF37" strokeWidth="12"
          ...
          transition={{ duration: 1.4, delay: 0.5, ease: [0.16, 1, 0.3, 1] }} strokeDashoffset={-hotD} strokeLinecap="round" />
        <motion.circle cx={cx} cy={cy} r={r} fill="none" stroke="#CBD5E1" strokeWidth="12"
          ...
          transition={{ duration: 1.4, delay: 0.7, ease: [0.16, 1, 0.3, 1] }} strokeDashoffset={-(hotD + warmD)} strokeLinecap="round" />
      </svg>
...
function LiveCallSimulator() {
  ...
  return (
    <div ref={ref} className="tilt-card h-full" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
      <div ref={innerRef}
        className="premium-card p-6 relative overflow-hidden h-full flex flex-col justify-center"
        style={{ transition: 'transform 0.15s ease-out' }}
      >
        <div className="tilt-shine rounded-[20px]" />

        {/* Animated background glow when calling */}
        {isCalling && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="absolute inset-0 rounded-[20px] pointer-events-none"
            style={{ background: "radial-gradient(circle at 20% 50%, rgba(31,138,112,0.06), transparent 70%)" }}
          />
        )}
        ...
        <div className="flex items-center justify-between mb-5 relative z-10">
            ...
            <div className="relative">
              ...
              {isCalling && [1, 2, 3].map(i => (
                <div key={i} className="absolute inset-0 rounded-2xl animate-ping"
                  style={{ backgroundColor: "#1F8A70", opacity: 0.15, animationDelay: `${i * 0.4}s` }} />
              ))}
            </div>
            <div>
              <div className="text-sm font-black" style={{ color: "#09090b", fontFamily: "'Outfit', sans-serif" }}>
                {isCalling ? "Active Intelligence" : "Sarvam AI Engine"}
              </div>
          ...
          <motion.button onClick={() => setIsCalling(!isCalling)}
        ...
        {/* Waveform visualizer */}
        <div className="relative h-16 rounded-2xl flex items-center justify-center gap-[3px] px-4 overflow-hidden border"
          style={{ background: "rgba(0,0,0,0.03)", borderColor: "rgba(0,0,0,0.06)" }}>
          {!isCalling ? (
            <div className="text-[10px] font-mono uppercase tracking-widest" style={{ color: "rgba(0,0,0,0.18)" }}>Standing by...</div>
          ) : (
            [...Array(32)].map((_, i) => {
              const h = 15 + Math.random() * 35;
              return (
                <motion.div key={i} animate={{ height: [h, h * 0.3, h * 1.3, h] }}
                  transition={{ duration: 0.5 + Math.random() * 0.7, repeat: Infinity, ease: "easeInOut" }}
                  className="w-[3px] rounded-full shrink-0"
                  style={{ backgroundColor: i % 2 === 0 ? "#1F8A70" : "#D4AF37", opacity: 0.7 }} />
              );
            })
          )}
        </div>
    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2 pointer-events-none" style={{ maxWidth: 300, width: '100%' }}>
      {toasts.map(t => (
        <motion.div key={t.id}
          ...
          style={{ background: 'white', border: `1px solid ${t.color}20`, boxShadow: `0 8px 32px rgba(0,0,0,0.12), 0 0 0 1px ${t.color}10` }}>
          ...
          {t.progress && (
            <div className="h-[3px]" style={{ backgroundColor: `${t.color}15` }}>
              <motion.div className="h-full" initial={{ width: '0%' }} animate={{ width: '100%' }}
                ...
                onAnimationComplete={() => setTimeout(() => remove(t.id), 100)}
                style={{ backgroundColor: t.color, borderRadius: '0 2px 2px 0' }} />
            </div>
          )}
        </motion.div>
      ))}
...
function downloadCSV() {
  const rows = [
    ['ID', 'Name', 'Company', 'Phone', 'Score', 'Bucket', 'Status', 'Date'],
    ['1', 'Priya Menon', 'Infosys', '+91 98765 43210', '9.1', 'HOT', 'COMPLETED', '2026-04-17'],
    ['2', 'Arjun Sharma', 'Wipro', '+91 87654 32109', '8.5', 'HOT', 'COMPLETED', '2026-04-17'],
    ['3', 'Neha Kapoor', 'TCS', '+91 76543 21098', '6.2', 'WARM', 'COMPLETED', '2026-04-16'],
    ['4', 'Rohit Verma', 'HCL Tech', '+91 65432 10987', '4.8', 'WARM', 'CALLING', '2026-04-16'],
    ['5', 'Anjali Singh', 'Cognizant', '+91 54321 09876', '2.1', 'COLD', 'COMPLETED', '2026-04-15'],
    ['6', 'Vikram Nair', 'Tech Mahindra', '+91 43210 98765', '-', '-', 'PENDING', '2026-04-15'],
    ['7', 'Kavya Reddy', 'Mphasis', '+91 32109 87654', '7.3', 'HOT', 'COMPLETED', '2026-04-15'],
    ['8', 'Siddharth Rao', 'Zensar', '+91 21098 76543', '5.5', 'WARM', 'COMPLETED', '2026-04-14'],
    ['9', 'Meera Iyer', 'L&T Infotech', '+91 10987 65432', '8.9', 'HOT', 'COMPLETED', '2026-04-14'],
    ['10', 'Ravi Kumar', 'Persistent', '+91 99876 54321', '1.8', 'COLD', 'FAILED', '2026-04-13'],
  ];
  const blob = new Blob([rows.map(r => r.join(',')).join('\n')], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = Object.assign(document.createElement('a'), { href: url, download: `voicequal_leads_${new Date().toISOString().split('T')[0]}.csv` });
...
function QuickActions() {
  ...
  const run = async (i: number) => {
    ...
    if (i === 0) {
      ...
      downloadCSV();
      push({ title: 'Export Complete', sub: '10 leads saved to voicequal_leads.csv', color: '#D4AF37' });
    } else if (i === 2) {
  ...
  const actions = [
    ...
    { icon: Send,      label: 'Export',    sub: 'Download CSV',  color: '#D4AF37', bg: 'rgba(212,175,55,0.08)' },
    { icon: RefreshCw, label: 'Sync CRM',  sub: 'HubSpot',       color: '#0F3D3E', bg: 'rgba(15,61,62,0.08)' },
    { icon: Zap,       label: 'Run Batch', sub: '6 leads',        color: '#A67C2E', bg: 'rgba(166,124,46,0.08)' },
  ];
            ...
            return (
              <motion.button key={i} onClick={() => run(i)} disabled={busy}
                ...
                style={{
                  ...
                  borderColor: busy ? `${act.color}35` : 'rgba(0,0,0,0.07)',
                  boxShadow: busy ? `0 0 0 2px ${act.color}15, 0 4px 16px ${act.color}10` : '0 1px 3px rgba(0,0,0,0.04)',
                  cursor: busy ? 'wait' : 'pointer',
                }}>
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity"
                  style={{ background: `radial-gradient(circle at 50% 50%, ${act.bg}, transparent 70%)` }} />
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-2 relative z-10"
                  style={{ backgroundColor: act.bg, border: `1px solid ${act.color}18` }}>
                  {busy
                    ? <motion.div animate={{ rotate: 360 }} transition={{ duration: 0.7, repeat: Infinity, ease: 'linear' }}>
                      <RefreshCw style={{ color: act.color, width: 16, height: 16 }} />
                    </motion.div>
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <motion.div {...fadeUp(0)} className="flex items-end justify-between">
          <div>
            ...
            <p className="text-sm mt-1.5 font-medium" style={{ color: "#71717a" }}>
              Real-time lead qualification performance — <span style={{ color: "#1F8A70", fontWeight: 700 }}>312 hot leads</span> ready for follow-up.
            </p>
          </div>
          <motion.div {...fadeUp(0.1)} className="hidden lg:flex items-center gap-4 px-5 py-3 rounded-2xl machined-panel">
            <div className="text-center">
              <div className="text-xl font-black tabular-nums" style={{ color: "#D4AF37", fontFamily: "'Outfit', sans-serif" }}>34.7%</div>
            ...
            <div className="text-center">
              <div className="text-xl font-black tabular-nums" style={{ color: "#1F8A70", fontFamily: "'Outfit', sans-serif" }}>9.1</div>
              <div className="text-[9px] font-black uppercase tracking-widest mt-0.5" style={{ color: "#94a3b8" }}>Top Score</div>
            </div>
            <div className="w-px h-10" style={{ backgroundColor: "rgba(212,175,55,0.15)" }} />
            <div className="text-center">
              <div className="text-xl font-black tabular-nums" style={{ color: "#0F3D3E", fontFamily: "'Outfit', sans-serif" }}>990</div>
              <div className="text-[9px] font-black uppercase tracking-widest mt-0.5" style={{ color: "#94a3b8" }}>Calls Today</div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="grid gap-5 lg:grid-cols-5">
          ...
          <motion.div {...fadeUp(0.33)} className="lg:col-span-2"><QuickActions /></motion.div>
        </div>
          ...
          <motion.div {...fadeUp(0.38)} className="lg:col-span-2 premium-card overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid rgba(212,175,55,0.08)" }}>
              <div>
                ...
                <div className="text-xs mt-0.5 font-medium" style={{ color: "#71717a" }}>Latest leads through the pipeline</div>
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3.5 flex-1 min-w-0">
                    <div className="relative shrink-0">
                      <div className="w-10 h-10 rounded-full flex items-center justify-center text-[11px] font-black text-white group-hover:scale-110 transition-transform"
                        style={{ backgroundColor: "#1F8A70", border: "1.5px solid rgba(212,175,55,0.3)", boxShadow: "0 2px 8px rgba(31,138,112,0.25)" }}>
                        {lead.name.slice(0, 2).toUpperCase()}
                      </div>
                      {lead.status === 'CALLING' && (
                        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white animate-pulse" style={{ backgroundColor: "#D4AF37" }} />
                      )}
                    </div>
                        ...
                        {lead.score != null && (
                          <>
                            ...
                            <span className="text-[10px] font-black tabular-nums"
                              style={{ color: lead.score >= 7 ? "#1F8A70" : lead.score >= 4 ? "#D4AF37" : "#94A3B8" }}>
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    ...
                    <ArrowRight className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" style={{ color: "#71717a" }} />
                  </div>
                </motion.div>
              );
            })}
          ...
          <div className="space-y-5">
            ...
            <motion.div {...fadeUp(0.45)} className="premium-card overflow-hidden">
              <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(212,175,55,0.08)" }}>
                ...
                <div className="text-xs mt-0.5 font-medium" style={{ color: "#71717a" }}>Current bucket breakdown</div>
              </div>
                    ...
                    <div key={idx}>
                      <div className="flex justify-between items-center mb-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-mono w-8 text-right" style={{ color: color }}>{stage.percentage}%</span>
                        </div>
                      </div>
                      <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: `${color}10` }}>
                        <motion.div initial={{ width: 0 }} animate={{ width: `${stage.percentage}%` }}
                          transition={{ duration: 1.2, delay: 0.5 + idx * 0.1, ease: [0.16, 1, 0.3, 1] }}
